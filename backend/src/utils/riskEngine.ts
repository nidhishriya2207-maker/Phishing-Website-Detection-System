...
export interface ScanDetailResult {
  ...
  metrics: {
    ...
    urlAnalysis: {
      length: number;
      isLengthSuspicious: boolean;
      hasIPAddress: boolean;
      subdomainCount: number;
      isSubdomainDepthSuspicious: boolean;
      hasAtSymbol: boolean;
...
interface LeafNode {
  value: number[]; // [votes_class_0, votes_class_1]
}
...
function isLeaf(node: any): node is LeafNode {
...
function predictRFProbability(forest: DecisionNode[], features: number[]): number {
...
let cachedForest: DecisionNode[] | null = null;

function loadModel(): DecisionNode[] | null {
...
export const computeRisk = async (urlString: string, isBlacklisted: boolean): Promise<ScanDetailResult> => {
  ...
  if (safeBrowsing.is_malicious) {
    return createImmediateThreatResult(
      urlString, 
      domain, 
      `Flagged by Google Safe Browsing as ${safeBrowsing.threat_type || 'malicious link'}`
    );
  }
  ...
  if (forest) {
    ...
    let fallbackScore = 0;
    if (!sslResult.https_enabled) fallbackScore += 25;
    if (!sslResult.ssl_valid) fallbackScore += 25;
    if (whoisResult.domain_age_days !== null) {
      if (whoisResult.domain_age_days < 30) fallbackScore += 25;
      else if (whoisResult.domain_age_days < 180) fallbackScore += 15;
    } else {
      fallbackScore += 20;
    }
    if (!dnsResult.dns_has_a_records) fallbackScore += 20;
    if (!dnsResult.has_ns_records) fallbackScore += 10;
...
const createImmediateThreatResult = (
  url: string,
  domain: string,
  reason: string
): ScanDetailResult => {
